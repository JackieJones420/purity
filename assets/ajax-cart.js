...
var ajaxCart = (function(module, $) {
  ...
  var gwp2 = new Holly.PromoItem(
    ...
    '/discount/URTHEBALM?redirect=%2Fproducts%2Fblood-orange-cleansing-balm',
    3900,
    5000
  );
  ...
  initRegularPromoMessage = function (){
    $('#PromoMessage').html(`
      <div class="p-sm">
        <a href="">
        <img
          src="https://cdn.no-toxic.com/e_vectorize:5:20,f_svg/v51/"
          data-src="https://cdn.no-toxic.com/q_auto:best,f_auto,fl_lossy,w_auto,c_limit,dpr_auto/v51/"
          class="cld-prepare"
  ...
  initGwpMessage2 = function (cart) {
    ...
    if (cartTotal < threshold || cart.item_count === 0) {
      ...
      if (inCart) {
        $targetDiv.html('<div class="p-xs bg-peach outer"><div class="p-sm inner"><div class="text-u-c p-sm text-center callout_text l-s">FREE GIFT IN TOTE</div><div class="caption_text text-center">Discount will be applied at checkout. </span></div></div></div>');
      } else {
        $targetDiv.html(`<a href=${gwp2.handle}> <div class="p-sm text-center text-white brand-bg-hightlight"> <div class="l-s text-u-c caption_text font-bold">Free Gift Unlocked!</div><div class="caption_text">${gwp2.title} ${gwp2.value>0 ? `<span class="caption_s_text">(${Shopify.formatMoney(gwp2.value, settings.moneyFormat)} value) </span>` : ''}</div><div class="m-t-sm"><span class="caption_s_text text-u-l l-s text-u-c font-bold">Add your gift!</span> <span class=caret-right></span></div></div></a>`);
      }
    }
  }
  ...
  buildCart = function (cart) {
    ...
    items = regularItems.concat(sampleItems);// convert cart total to points earned
    ...
    // console.log(data)
    if ( data.items.find( item => item.id == "32162856468558" ) ) {
      $('#textPromo').hide();
    } else {
      $('#textPromo').show();
    }
